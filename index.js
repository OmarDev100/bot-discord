...
client.on('ready', () => {
  ...
  let status = [`𝖡𝗒 Flash S`,`${prefix}help`];
  setInterval(()=>{
...
const owner = ["712690469540200501","712690469540200501"]///owner id
const supportServer = "https://discord.gg/dhYPJFcscg"///server support
const help_info_color = "blue"///color in help
const color = "blue"//main color
const moreInfo = `Need more info? use ${prefix}help`
const line = "https://probot.media/tOfZGU1KQS.gif"///url line link
const صح = "✅";//react
const غلط = "❌";//react
...
client.on('message', message => {
  ...
  if (message.channel.id !== '1035769713269145610') return;
...
message.lineReply(`
> \`-\` **My Link :** https://discord.gg/tuPySSatPS
...
client.on("message", message => {
  ...
  .setImage(line)
       ...
       .setColor(color)
  if (message.channel.id != "1032495814322434058") return;
...
client.on("message", message => {
  ...
  .setImage(line)
       ...
       .setColor(color)
  if (message.channel.id != "1032495803144622080") return;
...
///Line After transfer
 let channelid = '1032495815756881980'  
client.on('message', message => {
...
message.lineReply(embed).then(m=>{
       setTimeout(function(){
         m.edit(
           ...
           .setColor(color)
           .setImage(line)
           .setDescription(`**Welcome To __Server Name__** 
...
return message.channel.send(`not enought perms`)
    ...
    await user.setNickname(`𝑭𝒍𝒂𝒔𝒉丶 ${user.user.username}`,'idk').then(async () => {
...
///suggestions
let sug = ['1032495840012533770'];//channel sug
